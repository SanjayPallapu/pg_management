...
import { getCompatibleUpiApps, launchUpiPayment, NativePaymentError, openCameraSettings, scanUpiQr, scanUpiQrFromGallery } from "@/features/bill-payments/nativePayments";
...
type Stage = "entry" | "apps" | "result" | "receipt";
...
const messageForError = (error: unknown) => {
  ...
  if (error instanceof NativePaymentError) {
    ...
    if (error.code === "NO_UPI_APP") return "No compatible UPI app is installed on this phone.";
...
export const BillPaymentFlow = ({ open, request, onOpenChange }: Props) => {
  ...
  const prepareApps = async (parsed: ParsedUpiQr, selectedAmount: number) => {
    ...
    if (compatible.length === 0) throw new NativePaymentError("NO_UPI_APP");
  ...
  const sheetTitle = stage === "entry" ? "Add payment" : stage === "apps" ? "Choose UPI app" : stage === "result" ? "Confirm payment result" : "Payment recorded";

  return (
    <>
      <Sheet open={open} onOpenChange={(next) => !busy && onOpenChange(next)}>
        <SheetContent side="bottom" className="max-h-[94dvh] overflow-y-auto rounded-t-[28px] border-0 bg-[#f8f9fd] p-0 dark:bg-background [&>button]:hidden">
          ...
          <div className="mx-auto w-full max-w-lg space-y-4 px-3 py-4 sm:px-4">
            ...
            {error && <div role="alert" className="flex gap-3 rounded-2xl border border-rose-200 bg-rose-50 p-3 text-sm text-rose-900 dark:border-rose-900 dark:bg-rose-950/40 dark:text-rose-100">{error.includes("offline") ? <WifiOff className="h-5 w-5 shrink-0" /> : permissionDenied ? <Camera className="h-5 w-5 shrink-0" /> : <CircleAlert className="h-5 w-5 shrink-0" />}<div className="flex-1"><p className="font-bold">{error}</p>{permissionDenied && <button type="button" onClick={() => void openCameraSettings()} className="mt-2 min-h-11 rounded-xl bg-white px-3 text-xs font-black text-rose-700">Open app settings</button>}{qr && stage !== "receipt" && <button type="button" onClick={resetScan} className="mt-2 flex min-h-11 items-center gap-2 rounded-xl bg-white px-3 text-xs font-black text-rose-700"><RotateCcw className="h-4 w-4" /> Rescan QR</button>}</div></div>}
