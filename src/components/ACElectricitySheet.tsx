...
export const ACElectricitySheet = ({
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-xl p-0 [&>button]:hidden bg-[#070814] text-white border-l border-slate-900 flex flex-col h-full overflow-hidden">
        {selectedRoomItem ? (
          <ACRoomDetailView
          ...
          <div className="flex flex-col h-full overflow-hidden">
            {/* Header */}
            <SheetHeader className="px-5 pt-5 pb-3 border-b border-slate-900 shrink-0">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0 text-slate-400 hover:text-white hover:bg-slate-900" onClick={() => onOpenChange(false)} aria-label="Back">
                    <ArrowLeft className="h-5 w-5" />
                  </Button>
                  <div>
                    <div className="flex items-center gap-1.5">
                      <Snowflake className="h-4 w-4 text-cyan-400 shrink-0" />
                      <SheetTitle className="text-base font-extrabold text-white">AC Electricity Bill</SheetTitle>
                    </div>
                    <span className="text-[10px] text-slate-400 mt-0.5 block">Track. Split. Collect.</span>
                  </div>
                </div>
                <div className="flex items-center gap-1">
                  <select value={acMonth} onChange={(e) => setAcMonth(parseInt(e.target.value))} className="h-7 rounded-lg bg-slate-900 border border-slate-800 text-slate-300 px-2 text-[11px] font-semibold focus-visible:outline-none focus:ring-1 focus:ring-cyan-500 cursor-pointer">
                    {months.map((m) => (<option key={m.value} value={m.value} className="bg-[#070814] text-white">{m.label}</option>))}
                  </select>
                  <select value={acYear} onChange={(e) => setAcYear(parseInt(e.target.value))} className="h-7 rounded-lg bg-slate-900 border border-slate-800 text-slate-300 px-2 text-[11px] font-semibold focus-visible:outline-none focus:ring-1 focus:ring-cyan-500 cursor-pointer">
                    {years.map((y) => (<option key={y} value={y} className="bg-[#070814] text-white">{y}</option>))}
                  </select>
                </div>
              </div>
            </SheetHeader>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
              {/* Overview Card */}
              <div className="relative rounded-2xl overflow-hidden bg-gradient-to-br from-indigo-950/40 via-slate-900/50 to-cyan-950/20 border border-slate-800/80 p-5 shadow-lg">
                <div className="absolute right-0 top-0 w-32 h-full bg-gradient-to-l from-cyan-500/5 to-transparent pointer-events-none" />
                <h4 className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400 mb-3">Month Overview</h4>
                <div className="grid grid-cols-3 gap-3 mb-4">
                  <div className="bg-slate-950/50 border border-slate-800/60 rounded-xl p-3 text-center">
                    <span className="block text-[9px] uppercase tracking-wider font-extrabold text-slate-400">Expected</span>
                    <span className="block text-sm font-black text-cyan-300 mt-1">₹{expectedTotal.toLocaleString()}</span>
                  </div>
                  <div className="bg-slate-950/50 border border-slate-800/60 rounded-xl p-3 text-center">
                    <span className="block text-[9px] uppercase tracking-wider font-extrabold text-slate-400">Collected</span>
                    <span className="block text-sm font-black text-emerald-400 mt-1">₹{collectedTotal.toLocaleString()}</span>
                  </div>
                  <div className="bg-slate-950/50 border border-slate-800/60 rounded-xl p-3 text-center">
                    <span className="block text-[9px] uppercase tracking-wider font-extrabold text-slate-400">Pending</span>
                ...
                <div className="space-y-1.5">
                  ...
                  <Progress value={overallPct} className="h-1.5 bg-slate-950 border border-slate-800" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-4 gap-2">
                {[
                  ...
                  { icon: <FileSpreadsheet className="h-4 w-4" />, label: "Report", color: "text-emerald-400 bg-emerald-500/10 border-emerald-500/20", onClick: () => window.print() },
                ].map((action) => (
                  <button key={action.label} onClick={action.onClick} className="flex flex-col items-center justify-center p-3 bg-slate-900/40 border border-slate-900 rounded-xl hover:bg-slate-900/60 transition-colors gap-1.5">
                    <div className={cn("h-8 w-8 rounded-full border flex items-center justify-center", action.color)}>{action.icon}</div>
                    <span className="text-[9px] font-semibold text-slate-300">{action.label}</span>
                  </button>
                ))}
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between px-1">
                  <h3 className="text-xs font-extrabold uppercase tracking-wider text-slate-400">Rooms Summary</h3>
                  <Badge variant="outline" className="text-[9px] border-slate-800 text-slate-400 px-2 py-0">{acRooms.length} Rooms</Badge>
                </div>
                <div className="space-y-2.5">
                  {acRooms.length === 0 ? (
                    <div className="text-center text-slate-500 py-12 text-xs border border-dashed border-slate-900 rounded-2xl">No AC Rooms configured.</div>
                  ) : acRooms.map((item) => {
                    ...
                    return (
                      <div key={item.room.id} onClick={() => setSelectedRoomId(item.room.id)} className="group p-4 bg-slate-950/40 hover:bg-slate-900/30 border border-slate-900 hover:border-slate-800 rounded-2xl transition-all cursor-pointer flex items-center justify-between gap-4 shadow-sm">
                        <div className="flex items-center gap-3.5 min-w-0">
                          <div className="relative h-10 w-10 shrink-0">
                            ...
                            <div className="absolute inset-0 flex items-center justify-center text-[9px] font-black text-white">{roomPct}%</div>
                          </div>
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              <span className="font-extrabold text-sm text-white">Room {item.room.roomNo}</span>
                            ...
                            <span className="text-[10px] text-slate-400 block mt-0.5">{item.room.capacity} Sharing · {item.activeTenants.length} Tenant{item.activeTenants.length === 1 ? '' : 's'}</span>
                            <span className="text-[9px] text-slate-500 block mt-1">{item.units || 0} Units · ₹{item.unitPrice}/Unit · Bill: ₹{(item.total || 0).toLocaleString()}</span>
                          </div>
                        </div>
                        <div className="flex items-center gap-1 text-slate-500 group-hover:text-slate-300 transition-colors shrink-0">
            ...
            <div className="border-t border-slate-900 bg-slate-950/80 backdrop-blur-md p-4 flex items-center justify-between shrink-0 text-xs">
              <div className="flex items-center gap-1.5">
...
const ACRoomDetailView = ({ item, onBack, onSaveReading, onShare, onTogglePaymentStatus, onModeToggle }: DetailProps) => {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <SheetHeader className="px-5 pt-5 pb-3 border-b border-slate-900 shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 min-w-0">
            <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0 text-slate-400 hover:text-white hover:bg-slate-900" onClick={onBack}>
              <ArrowLeft className="h-5 w-5" />
            </Button>
            <div className="min-w-0">
              <SheetTitle className="text-base font-extrabold text-white">Room {room.roomNo}</SheetTitle>
              <span className="text-[10px] text-slate-400 block mt-0.5">{room.capacity} Sharing · {activeTenants.length} Active Tenant{activeTenants.length === 1 ? '' : 's'}</span>
            </div>
          </div>
          <Button size="sm" variant="outline" className="h-7 text-[10px] border-slate-800 text-slate-300 hover:bg-slate-900 hover:text-white shrink-0 px-2.5" onClick={() => onShare(draftUnits, draftUnitPrice, startVal, endVal, selectedSplitType, draftSplitCount)}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        {/* Summary card */}
        <div className="rounded-2xl border border-slate-900 bg-slate-950/30 p-4 flex flex-col gap-3">
          <div className="flex justify-between items-center">
            <div><span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider block">Total Bill</span><span className="text-lg font-black text-white mt-0.5">₹{draftTotal.toLocaleString()}</span></div>
            <div className="text-right"><span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider block">Pending</span><span className={cn("text-lg font-black mt-0.5", roomPending > 0 ? "text-orange-400" : "text-emerald-400")}>₹{roomPending.toLocaleString()}</span></div>
          </div>
          <div className="space-y-1.5 border-t border-slate-900/60 pt-3">
            ...
            <Progress value={roomPct} className="h-1.5 bg-slate-950 border border-slate-900" />
          </div>
        </div>

        {/* Meter Reading */}
        <div className="rounded-2xl border border-slate-900 bg-slate-950/30 p-4 space-y-3.5">
          <div className="flex items-center justify-between">
            <h4 className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Meter Readings</h4>
            <div className="flex items-center gap-1.5">
              <input type="checkbox" id={`detail-ac-mode-${room.roomNo}`} checked={isCustom} onChange={(e) => onModeToggle(e.target.checked)} className="h-3.5 w-3.5 rounded border-slate-800 bg-slate-950 text-cyan-600 cursor-pointer" />
              <label htmlFor={`detail-ac-mode-${room.roomNo}`} className="text-[10px] text-slate-400 cursor-pointer">Flat Rate (₹{draftUnitPrice}/unit)</label>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label className="text-[10px] font-extrabold uppercase text-slate-500 block mb-1">Prev Reading</Label>
              <Input type="number" value={startReadingDraft} onChange={(e) => setStartReadingDraft(e.target.value)} onBlur={handleStartBlur} onKeyDown={(e) => { if (e.key === "Enter") e.currentTarget.blur(); }} placeholder="Start" className="h-9 text-xs px-2.5 bg-slate-950 border-slate-900 text-white focus-visible:ring-cyan-500" />
            </div>
            <div>
              <Label className="text-[10px] font-extrabold uppercase text-slate-500 block mb-1">Curr Reading</Label>
              <Input type="number" value={endReadingDraft} onChange={(e) => setEndReadingDraft(e.target.value)} onBlur={handleEndBlur} onKeyDown={(e) => { if (e.key === "Enter") e.currentTarget.blur(); }} placeholder="End" className="h-9 text-xs px-2.5 bg-slate-950 border-slate-900 text-white focus-visible:ring-cyan-500" />
            </div>
            <div>
              <Label className="text-[10px] font-extrabold uppercase text-slate-500 block mb-1">Units Consumed</Label>
              <Input type="number" value={unitsDraft} onChange={(e) => setUnitsDraft(e.target.value)} onBlur={handleUnitsBlur} onKeyDown={(e) => { if (e.key === "Enter") e.currentTarget.blur(); }} placeholder="0" className="h-9 text-xs px-2.5 bg-slate-950 border-slate-900 text-white focus-visible:ring-cyan-500" />
            </div>
            <div>
              <Label className="text-[10px] font-extrabold uppercase text-slate-500 block mb-1">Rate per Unit (₹)</Label>
              <Input type="number" value={priceDraft} disabled={!isCustom} onChange={(e) => setPriceDraft(e.target.value)} onBlur={handlePriceBlur} onKeyDown={(e) => { if (e.key === "Enter") e.currentTarget.blur(); }} className="h-9 text-xs px-2.5 bg-slate-950 border-slate-900 text-white disabled:opacity-40 focus-visible:ring-cyan-500" />
            </div>
          </div>
        </div>
        ...
        {/* Split Strategy */}
        <div className="rounded-2xl border border-slate-900 bg-slate-950/30 p-4 space-y-3.5">
          <h4 className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Bill Split Strategy</h4>
          <div className="grid grid-cols-2 gap-3 items-end">
            <div>
              <Label className="text-[10px] font-extrabold uppercase text-slate-500 block mb-1">Strategy</Label>
              <select value={selectedSplitType} onChange={(e) => handleSplitTypeChange(e.target.value)} className="h-9 rounded-lg border border-slate-900 bg-slate-950 px-2 text-xs font-semibold focus-visible:outline-none focus:ring-1 focus:ring-cyan-500 text-white w-full cursor-pointer">
            ...
            <div>
              {selectedSplitType === "custom" ? (
                <>
                  <Label className="text-[10px] font-extrabold uppercase text-slate-500 block mb-1">Persons Count</Label>
                  <Input type="number" min="1" value={splitCountDraft} onChange={(e) => setSplitCountDraft(e.target.value)} onBlur={handleSplitCountBlur} onKeyDown={(e) => { if (e.key === "Enter") e.currentTarget.blur(); }} placeholder="Count" className="h-9 text-xs px-2.5 bg-slate-950 border-slate-900 text-white focus-visible:ring-cyan-500" />
                </>
              ) : (
                <div className="h-9 flex flex-col justify-center">
                  <span className="text-[10px] font-extrabold uppercase text-slate-500 block">Total</span>
                  <span className="text-xs font-black text-cyan-400 mt-0.5">₹{draftTotal.toLocaleString()}</span>
                </div>
              )}
            </div>
          </div>
          <div className="flex items-center justify-between rounded-xl bg-slate-900/50 border border-slate-900 px-3 py-2 text-[11px] text-slate-400">
        ...
        {/* Tenants */}
        <div className="space-y-2.5">
          <h4 className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400 px-1">Tenant Breakdown</h4>
          <div className="grid gap-2">
            {dayWiseShares.length === 0 ? (
              <div className="text-center text-slate-500 py-6 text-xs border border-dashed border-slate-900 rounded-xl">No active tenants in this room.</div>
              ...
              return (
                <div key={tenant.name} className="p-3.5 bg-slate-950/40 border border-slate-900 hover:border-slate-800/80 rounded-2xl flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className={cn("h-9 w-9 rounded-full border flex items-center justify-center text-xs font-bold shrink-0", isPaid ? "bg-emerald-950/50 border-emerald-800/60 text-emerald-400" : "bg-slate-900 border-slate-800 text-slate-300")}>{initials}</div>
                    <div className="min-w-0">
                      <span className="font-extrabold text-xs text-white truncate block">{tenant.name}</span>
                      <span className="text-[10px] text-slate-400 block mt-0.5">{tenant.daysStayed} days stayed</span>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    {!isPaid && (
                      <Button size="xs" variant="outline" className="h-7 w-7 p-0 rounded-lg border-slate-800 text-slate-400 hover:bg-slate-900 hover:text-white" onClick={() => onShare(draftUnits, draftUnitPrice, startVal, endVal, selectedSplitType, draftSplitCount, tenant.name)}>
                    ...
                    <Button
                      ...
                      className={cn("h-7 px-3 text-[10px] font-bold rounded-lg border-0",
                        isPaid ? "bg-slate-900 hover:bg-slate-800 text-emerald-400" : "bg-cyan-500 hover:bg-cyan-600 text-slate-950"
                      )}
      ...
      <div className="border-t border-slate-900 bg-slate-950/80 backdrop-blur-md p-4 flex items-center gap-2 shrink-0">
        <Button variant="outline" className="flex-1 text-[11px] font-bold h-9 border-slate-800 bg-slate-900/50 text-slate-300 hover:bg-slate-900 hover:text-white" onClick={() => onShare(draftUnits, draftUnitPrice, startVal, endVal, selectedSplitType, draftSplitCount)}>
