...
import { Calendar, MessageCircle, Phone, User, ArrowLeft } from 'lucide-react';
...
export const ExpectedCollectionCard = ({ 
  ...
  const collectionScheduleData = useMemo(() => {
    ...
    allTenants.forEach(tenant => {
      ...
      const dueDay = hasAgreedDelay ? tenant.paymentDueDay! : joinDay;

      if (!scheduleByDay[dueDay]) {
        scheduleByDay[dueDay] = { 
          day: dueDay, 
          expected: 0, 
          tenants: 0, 
          delayedCount: 0,
      ...
      if (hasAgreedDelay) {
        scheduleByDay[dueDay].delayedCount++;
        scheduleByDay[dueDay].delayedExpected += balance;
      }
  ...
  const openWhatsAppChat = (phone: string, tenantName?: string, balance?: number, roomNo?: string, hasAgreedDelay?: boolean, paymentDueDay?: number | null) => {
    const formattedPhone = phone.replace(/\D/g, '');
    const phoneWithCode = formattedPhone.startsWith('91') ? formattedPhone : `91${formattedPhone}`;
    let msg = '';
    if (tenantName && balance && roomNo) {
      if (hasAgreedDelay && paymentDueDay) {
  ...
  const filteredDelayedCount = useMemo(() => filteredData.reduce((sum, item) => sum + item.delayedCount, 0), [filteredData]);
  const filteredDelayedTotal = useMemo(() => filteredData.reduce((sum, item) => sum + item.delayedExpected, 0), [filteredData]);
                  ...
                  <div>
                    ...
                    {filteredDelayedCount > 0 && (
                      <div className="text-[11px] text-purple-600 dark:text-purple-400 font-medium mt-0.5">
                        Includes {filteredDelayedCount} agreed delay tenant(s) (₹{filteredDelayedTotal.toLocaleString()})
                      </div>
                    )}
                  </div>
                      ...
                      >
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground">
                              {item.tenants} tenant(s) pending
                              {item.delayedCount > 0 && (
                                <span className="text-purple-600 dark:text-purple-400 font-medium"> • {item.delayedCount} agreed delay</span>
                              )}
                            </div>
                          </div>
                        </div>
                            ...
                            <div className="flex-1 min-w-0 text-left">
                              <div className="flex items-center gap-1.5 flex-wrap">
                                ...
                                {tenant.hasAgreedDelay && (
                                  <Badge
                                    variant="outline"
                                    className="h-4 px-1 text-[9px] border-purple-300 text-purple-700 bg-purple-50 dark:bg-purple-950/40 dark:text-purple-300 dark:border-purple-800"
                                  >
                              ...
                              <div className="text-[10px] text-muted-foreground">
                                ...
                                {tenant.hasAgreedDelay && ` • Joined Day ${tenant.joinDay}`}
                              </div>
                            </div>
                            {tenant.phone && tenant.phone !== '••••••••••' && (
                              <div className="flex items-center gap-1.5 shrink-0 ml-auto">
                                <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    openWhatsAppChat(tenant.phone, tenant.name, tenant.balance, tenant.roomNo, tenant.hasAgreedDelay, tenant.paymentDueDay);
                                  }}
