...
export const MonthlyRentSheet = ({
  ...
  const previousMonthOverdue = useMemo(() => {
    ...
    const prevMonthActiveTenants = allTenants.filter(tenant => isTenantActiveInMonth(tenant.startDate, tenant.endDate, prevYear, prevMonth));
  ...
  const handleDeletePayments = (entriesToDelete: number[], newAmountPaid: number, newEntries: PaymentEntry[]) => {
    ...
    const newStatus = newAmountPaid >= deletePaymentTenant.monthlyRent ? "Paid" : newAmountPaid > 0 ? "Partial" : "Pending";
    ...
    toast({
      ...
      description: newAmountPaid > 0 ? `Balance to pay: ₹${(deletePaymentTenant.monthlyRent - newAmountPaid).toLocaleString()}` : "Status updated to Pending"
    });
  ...
  return <div className="space-y-4">
      <Card>
        <CardHeader className="pb-3 px-3 pt-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              {leftTenantsCount > 0 && <Button variant="outline" size="sm" onClick={() => setCleanupSheetOpen(true)} className="h-7 px-2 text-xs text-destructive border-destructive/50 hover:bg-destructive/10 mr-[15px]" title="Left Tenants Cleanup">
        ...
        <CardContent className="px-3 pb-4">
          ...
          <div className="mb-4 relative">
            ...
            <Input placeholder="Search by tenant name or room..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-9 pr-9" />
            ...
            return <div key={tenant.id} className={cn("p-3 rounded-xl transition-all duration-200", bgClass)}>
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      {tenant.phone && tenant.phone !== "••••••••••" && <DropdownMenu>
                          ...
                          <DropdownMenuContent align="start">
                            {(tenant.payment.paymentStatus === "Paid" || tenant.payment.paymentStatus === "Partial") && <DropdownMenuItem onClick={handleResendReceipt} className="gap-2">
                            ...
                            {tenant.payment.paymentStatus !== "Paid" && <DropdownMenuItem onClick={openPaymentReminder} className="gap-2">
                    ...
                    {isPartial ? <Badge className="bg-overdue text-overdue-foreground">₹{remaining.toLocaleString()}</Badge> : <div className="font-semibold text-sm">₹{tenant.monthlyRent.toLocaleString()}</div>}
                  </div>
      ...
      <DeletePaymentDialog open={!!deletePaymentTenant} onOpenChange={open => !open && setDeletePaymentTenant(null)} tenantName={deletePaymentTenant?.name || ""} monthlyRent={deletePaymentTenant?.monthlyRent || 0} paymentEntries={deletePaymentTenant?.paymentEntries || []} onConfirmDelete={handleDeletePayments} />
      ...
      <PaymentReminderDialog open={reminderDialogOpen} onOpenChange={setReminderDialogOpen} reminderData={reminderData} />
