...
export const TenantRentCard = ({
  ...
  return (
    <div className={cn("transition-all duration-200 shadow-sm p-2 rounded-2xl", cardDesignClass)}>
      {/* Top Row: Name • Room No | Joined Date */}
      ...
      {tenant.isProRata && tenant.daysStayed && tenant.effectiveRent !== undefined && (
        <Collapsible open={showCalendar} onOpenChange={setShowCalendar} className="mt-1.5 px-1">
      ...
      {((tenant.payment.paymentEntries && tenant.payment.paymentEntries.length > 0) || isPaid) && (
        <div className="mt-1.5 space-y-0.5 px-1">
          <div className={cn("text-xs font-medium", !isPaid ? "font-bold text-emerald-600 dark:text-emerald-400" : "text-slate-500 dark:text-slate-400")}>
          ...
          {tenant.payment.paymentEntries && tenant.payment.paymentEntries.length > 0 ? (
            tenant.payment.paymentEntries.map((entry, idx) => (
              <div key={idx} className="flex flex-wrap items-center gap-1.5 text-xs text-slate-600 dark:text-slate-300">
      ...
      {/* ONE DIV containing: Amount + (Chat & Call badges) + Pay/Paid Button with space-between */}
      <div className="mt-2 flex items-center justify-between w-full pt-1 px-1 gap-2">
        {/* 1. Amount */}
        {isPaid ? (
