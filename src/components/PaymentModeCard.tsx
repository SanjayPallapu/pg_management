...
export const PaymentModeCard = () => {
  ...
  const stats = useMemo(() => {
    ...
    const monthPayments = payments.filter(p => p.month === selectedMonth && p.year === selectedYear && eligibleTenantIds.has(p.tenantId));
    let upiCount = 0;
    let upiAmount = 0;
    let cashCount = 0;
    let cashAmount = 0;
            ...
            <Tooltip>
              ...
              <TooltipContent side="bottom" className="max-w-xs p-3">
                <div className="space-y-2">
                  ...
                  {stats.upiDetails.length > 0 ? <div className="space-y-1 max-h-40 overflow-y-auto">
                      {stats.upiDetails.slice(0, 10).map((detail, idx) => <div key={idx} className="flex justify-between text-xs gap-4">
                          <span className="truncate">{detail.tenantName}</span>
                          <span className="font-medium whitespace-nowrap">
                            ₹{detail.amount.toLocaleString()} • {format(new Date(detail.date), 'dd MMM')}
                          </span>
                        </div>)}
                      {stats.upiDetails.length > 10 && <div className="text-xs text-muted-foreground text-center pt-1">
                          +{stats.upiDetails.length - 10} more
                        </div>}
                    </div> : <div className="text-xs text-muted-foreground">No UPI payments</div>}
                </div>
              </TooltipContent>
            </Tooltip>
            ...
            <Tooltip>
              ...
              <TooltipContent side="bottom" className="max-w-xs p-3">
                <div className="space-y-2">
                  ...
                  {stats.cashDetails.length > 0 ? <div className="space-y-1 max-h-40 overflow-y-auto">
                      {stats.cashDetails.slice(0, 10).map((detail, idx) => <div key={idx} className="flex justify-between text-xs gap-4">
                          <span className="truncate">{detail.tenantName}</span>
                          <span className="font-medium whitespace-nowrap">
                            ₹{detail.amount.toLocaleString()} • {format(new Date(detail.date), 'dd MMM')}
                          </span>
                        </div>)}
                      {stats.cashDetails.length > 10 && <div className="text-xs text-muted-foreground text-center pt-1">
                          +{stats.cashDetails.length - 10} more
                        </div>}
                    </div> : <div className="text-xs text-muted-foreground">No cash payments</div>}
                </div>
              </TooltipContent>
            </Tooltip>
          </div>
